...
function AdminProductPage() {
    ...
    useEffect(() => {
        const categoriesRef = ref(database, "products");
    ...
    const addCategory = () => {
        ...
        const categoryRef = push(ref(database, `products`));
    ...
    const deleteCategory = (categoryId) => {
        const categoryRef = ref(database, `products/${categoryId}`);
        remove(categoryRef).catch((error) =>
    ...
    const addBrand = () => {
        ...
        const brandRef = push(ref(database, `products/${selectedCategory}/brands`));
    ...
    const updateBrand = (categoryId, brandId) => {
        const brandRef = ref(database, `products/${categoryId}/brands/${brandId}`);
    ...
    // Ürün Ekleme
    const addProduct = () => {
        if (!selectedCategory || !selectedBrand || newProductName.trim() === "")
            return;
        const productsRef = push(
            ref(
                database,
                `products/${selectedCategory}/brands/${selectedBrand}/products`
            )
        );
    ...
    const updateProduct = (categoryId, brandId, productId, updatedProduct) => {
        const productRef = ref(
            database,
            `products/${categoryId}/brands/${brandId}/products/${productId}`
        );
        update(productRef, updatedProduct).catch((error) =>
                                    ...
                                    >
                                        {editingCategory === categoryId ? (
                                            <div>
                                                ...
                                                />
                                                <button onClick={() => updateCategory(categoryId)}>
                                                    Kaydet
                                                </button>
                                            ...
                                            <div>
                                                <h3 onClick={() => setSelectedCategory(categoryId)}>
                                                    {category.name}
                                                </h3>
                                                <button onClick={() => setEditingCategory(categoryId)}>
                                                    Düzenle
                                                </button>
                                        ...
                                        {selectedCategory === categoryId && (
                                            <div className="brands">
                                                <h4>Markalar</h4>
                                                {category.brands ? (
                                                    Object.entries(category.brands).map(
                                                        ([brandId, brand]) => (
                                                            <div
                                                                key={brandId}
                                                                style={{
                                                            ...
                                                            >
                                                                {editingBrand === brandId ? (
                                                                    <div>
                                                                        ...
                                                                        <button
                                                                            onClick={() =>
                                                                                updateBrand(categoryId, brandId)
                                                                            }
                                                                        >
                                                                            Kaydet
                                                                        </button>
                                                                    ...
                                                                    <div>
                                                                        <h5>{brand.name}</h5>
                                                                        <button
                                                                            onClick={() => setEditingBrand(brandId)}
                                                                        >
                                                                            Düzenle
                                                                        </button>
                                                                        <button
                        ...
                        {selectedCategory && selectedBrand && (
                            <div className="products">
                                <h3>
                                    ...
                                    {categories[selectedCategory]?.brands[selectedBrand]?.name}{" "}
                                    Ürünleri
                                </h3>
